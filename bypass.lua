...
task.spawn(function()
    ...
    local Amplitude = 10000
    local RandomValue = {-10000,10000}
    local RandomTime = {.1, 10}
    ...
    for i,v in next, RandomTime do
        RandomTime[i] = v * 30000
    end

    local RandomTimeValue = math.random(RandomTime[1],RandomTime[2])/30000
    ...
    while wait(RandomTime) do
        ...
        RandomTimeValue = math.random(RandomTime[1],RandomTime[2])/30000
    end
end)
...
task.spawn(function()
    ...
    local _MemBypass
    _MemBypass = hookmetamethod(game, "__moonnamecall", function(self,...)
...
ContentProviderBypass = hookmetamethod(game, "__moonnamecall", function(self, Instances, ...)
    local method = getnamecallmethod();
    local args = ...;
...
local TextboxBypass
TextboxBypass = hookmetamethod(game, "__moonnamecall", function(self,...)
    local method = getnamecallmethod();
    local args = ...;
    ...
    if not checkcaller() then
        if typeof(self) == "Instance" and method == "GetFocusedTextBox" and self.ClassName == "UserInputService" then
            ...
            if Textbox and typeof(Textbox) == "Instance" then
                local succ,err = pcall(function() _IsDescendantOf(Textbox, Moons_Bypassed_Dex) end)
